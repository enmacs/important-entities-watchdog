...
- **Repository:** enmacs/important_entities_watchdog

**Key directories:**

- `custom_components/important_entities_watchdog/` - Main integration code
...
- Integration logs: `custom_components.important_entities_watchdog: debug` in `config/configuration.yaml`
...
- Use the domain `important_entities_watchdog` for all DOMAIN references
- Prefix all integration-specific classes with `ImportantEntitiesWatchdog`
- Use "Important Entities Watchdog" as the display title
...
- Inherit from platform base + `ImportantEntitiesWatchdogEntity`
...
- `tests/` mirrors `custom_components/important_entities_watchdog/` structure
