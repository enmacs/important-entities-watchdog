...
This document describes all configuration options and settings available in the Important Entities Watchdog custom integration.
...
2. Find "Important Entities Watchdog"
3. Click **Configure**
4. Modify settings
5. Click **Submit**
...
### `important_entities_watchdog.example_service`
...
```yaml
service: important_entities_watchdog.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: important_entities_watchdog.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Important Entities Watchdog Alert
...
- [GitHub Issues](https://github.com/enmacs/important_entities_watchdog/issues) - Report problems
